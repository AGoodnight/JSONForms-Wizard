...
import { Position } from "components/ArtBoard/shapes.models";
...
const useCursorPosition = () => {
  ...
  const getPosition = useCallback((event: MouseEvent): number[] => {
    return [event.clientX, event.clientY];
  }, []);

  const getStagePosition = useCallback(
    (e: KonvaEventObject<MouseEvent>): Position | null => {
